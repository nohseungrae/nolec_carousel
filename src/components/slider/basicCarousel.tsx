...
export const BasicCarousel: React.FC<IBasicCarouselProps> = ({ stories, imgDomain, imgLegacy }) => {
    ...
    return (
        <>
            {state?.displayState === 'none' ? null : (
                <Popup state={state} closePopup={closePopup} imgDomain={imgDomain} imgLegacy={imgLegacy} />
            )}
            <div className="story_main flex overflow-x-auto max-w-full mr-auto">
                <ul className="flex justify-center flex-grow" style={{ padding: '0px 3% 3%' }}>
                    {state.groupedSlides.map((slide, i) => {
                        return (
                            ...
                            >
                                <span className="absolute w-full z-10 text-center text-white" style={{ bottom: '15px' }}>
                                    {slide.alt}
                                </span>
                                <img className="absolute -bottom-0 z-10" src={`/static/m/images/story_overlay.png`} alt={'overlay'} />
                                {checkVideoImg(
